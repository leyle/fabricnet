...
do
    ...
    fabric-ca-client register -d --id.name $PEER_NAME --id.secret $PEER_PASSWD --id.type peer -u https://${TLS_CA_HOST_PORT}
    ...
    fabric-ca-client register -d --id.name $PEER_NAME --id.secret $PEER_PASSWD --id.type peer -u https://${ECERT_CA_HOST_PORT}
